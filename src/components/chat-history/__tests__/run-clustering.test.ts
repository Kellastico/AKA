...
describe("clusterNodes", () => {
  ...
  it("keeps reasoning between tool clusters; each cluster is grouped independently", () => {
...
describe("groupSummary", () => {
  ...
  it("uses a kind-aware verb when the group is homogeneous", () => {
    ...
    expect(groupSummary([mk("run"), mk("run")])).toBe("Ran 2 commands");
    expect(groupSummary([mk("write"), mk("write")])).toBe("Edited 2 files");
    expect(groupSummary([mk("search"), mk("search")])).toBe("Ran 2 searches");
  });
